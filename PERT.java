...
public class PERT {
    ...
    public static Activities[] criticalPath(Map<String, Activities> activities) {   
        HashSet<String> completed = new HashSet<>();        //is empty FOR NOW
        HashSet<String> remaining = new HashSet<>(activities.keySet());     //stores the keys for all the elements in the map
        while (!remaining.isEmpty()) {  //loop while there are still elements in the remaining HashSet
            ...
            for (String activityName : remainingCopy) {  //for each taskName in remainingCopy HashSet, do the following
                Activities activity=activities.get(activityName);//returns the value corresponding to activityName which is a key
                if (completed.containsAll(activity.dependencies.stream().map(a -> a.name).toList())) {
                    //used to map each element in dependencies to corresponding name and converting it all to a list 
